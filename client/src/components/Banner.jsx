  ...
  return (                                                         
    <div className="w-full mb-10">                                         
        ...
        flex px-0 lg:px-20">                                       
             ...
             bottom-10 md:bottom-2 w-full md:w-2/4 lg:w-1/3 2xl:w-
             [480px] bg-white dark:bg-[#05132b] shadow-2xl
             rounded-lg gap-3 p-4">
                   ...
                   dark:text-slate-500 text-sm text-justify">
                    <Markdown option ={{wrapper:"article"}}>
                      {post?.desc?.slice(0,160)+ "..."}
                    </Markdown>

                   </div>
                   <Link to={`/${post?.slug}/${post?._id}`}
                   className="w-fit bg-rose-600 bg-opacity-20 text-red-700
                   ...
                   dark:text-slate-400">{post?.user?.name}
                   </span>
                    
                   <span className="text-gray-500 dark:text-gray-600">{new Date(post?.createdAt).toDateString()}
                   </span>
                   </Link>
             </div>
        </div>
